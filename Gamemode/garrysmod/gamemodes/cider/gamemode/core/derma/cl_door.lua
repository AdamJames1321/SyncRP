...
function PANEL:Init()
	...
	self.okay.DoClick = function()
		RunConsoleCommand( "cider", "door", "name", self.textEntry:GetValue() );
	end;
...
function PANEL:PerformLayout()
	...
	-- Check if we are the owner.
	if (owner) then
		self.purchase:SetText("Sell ($"..(cider.configuration["Door Cost"] / 2)..")");
		...
		if (self.owner) then
			...
			self.label:SetText("This door has already been sold.");
			...
			self.label:SetText("Purchase this door for $"..cider.configuration["Door Cost"]..".");
			self.label:SizeToContents();
...
usermessage.Hook("cider_Door", function(msg)
	...
	if ( IsValid(owner) ) then
		cider.door.panel.owner = owner:Name();
		cider.door.panel.textEntry:SetValue( door:GetNetworkedString("cider_Name") );
	end;
	...
	if (LocalPlayer() == owner) then
		for k, v in pairs(players) do
			if (v.access == 0) then
				local id = cider.door.panel.playerList:AddLine( v.player:Name() ):GetID();
				
				-- Add it to our list of players.
				cider.door.panel.playerList.players[id] = {name = v.player:Name(), entID = v.entID};
			elseif (v.access == 1) then
				local id = cider.door.panel.accessList:AddLine( v.player:Name() ):GetID();
				
				-- Add it to our list of players.
				cider.door.panel.accessList.players[id] = {name = v.player:Name(), entID = v.entID};
			end;
		end;
		
		-- Set the function to do a double click for the player list.
		cider.door.panel.playerList.DoDoubleClick = function(self, id, line)
			local index = cider.door.panel.accessList:AddLine(self.players[id].name):GetID();
			
			-- Add it to our list of players.
			cider.door.panel.accessList.players[index] = self.players[id];
			
			-- Remove the line from the players list.
			self:RemoveLine(id)
			
			-- Run a console command to tell the server we've changed access.
			RunConsoleCommand("cider", "door", "access", self.players[id].entID);
		end;
		
		-- Set the function to do a double click for the access list.
		cider.door.panel.accessList.DoDoubleClick = function(self, id, line)
			local index = cider.door.panel.playerList:AddLine(self.players[id].name):GetID();
			
			-- Add it to our list of players.
			cider.door.panel.playerList.players[index] = self.players[id];
			
			-- Remove the line from the access list.
			self:RemoveLine(id)
			
			-- Run a console command to tell the server we've changed access.
			RunConsoleCommand( "cider", "door", "access", tostring(self.players[id].entID) );
		end;
	else
		cider.door.panel.playerList:SetVisible(false);
		cider.door.panel.accessList:SetVisible(false);
	end;
end);