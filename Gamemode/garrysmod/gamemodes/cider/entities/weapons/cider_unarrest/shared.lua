...
if (SERVER) then
	...
	resource.AddFile("models/weapons/w_stunstick.phy");
...
SWEP.Instructions = "Primary Fire: Unarrest.";
SWEP.Contact = "http://kudomiku.com/forums/.";
SWEP.Purpose = "Unarresting people who have been imprisoned.";
	...
	if (trace.Entity and trace.Entity:IsPlayer() and self.Owner:GetPos():Distance(trace.HitPos) <= 128) then
		if (trace.Entity.cider._Arrested) then
			if ( hook.Call("PlayerCanUnarrest", GAMEMODE, self.Owner, trace.Entity) ) then
				...
				cider.player.printConsoleAccess(self.Owner:Name().." unarrested "..trace.Entity:Name()..".", "a");
				
				-- Call a hook.
				hook.Call("PlayerUnarrest", GAMEMODE, self.Owner, trace.Entity);
			end;
		end;
	end;
end;
