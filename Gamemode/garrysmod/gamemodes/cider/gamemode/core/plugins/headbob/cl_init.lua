...
CreateClientConVar("cider_headbob", 0, true, true);

-- Called when the view should be calculated.
cider.hook.add("CalcView", function(player, origin, angles, fov)
	if (GetConVarNumber("cider_headbob") == 1) then
		...
		or player:KeyDown(IN_MOVELEFT) ) and player:IsOnGround() ) then
			...
			if ( player:GetVelocity():Length() > (cider.configuration["Run Speed"] - 25) ) then
