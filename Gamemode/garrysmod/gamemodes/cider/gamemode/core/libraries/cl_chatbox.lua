...
function cider.chatBox.chatText(index, name, text, filter)
	...
	if (filter == "arrested" or filter == "yell" or filter == "whisper"
	or filter == "me" or filter == "advert" or filter == "request" or filter == "radio"
	...
	if ( IsValid(player) ) then
		...
		elseif ( player:GetNetworkedBool("cider_Moderator") ) then
			icon = {"gui/silkicons/emoticon_smile", ":)"};
		elseif ( player:GetNetworkedBool("cider_Donator") ) then
		...
		elseif (class == "pm") then
			cider.chatBox.messageAdd( {"(PM)"}, nil, { name..": "..text, Color(255, 150, 125, 255) }, filtered);
